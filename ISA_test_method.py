...
def To_Excel(t):
    f = open(r'train\\Movielens_ratings_test_%d.txt'%t,'r')
    lines = f.readlines()

    workbook = xlsxwriter.Workbook(r'train\\Movielens_test_%d.xlsx'%t)
...
def Test_split(t):
    t1 = time.time()
     ...
     # 每个群组的用户词典
    group_dict = xlrd.open_workbook('train\\Movielens_test_%d.xlsx'%t,'rb')
    t_test = group_dict.sheet_by_name('test')
    nrows = t_test.nrows

    workbook = xlsxwriter.Workbook(r'train\\Movielens_test_split_%d.xlsx'%(t))
