...
def ISA_group_ave_Pearson(f,t,k,uid,x):
    # t 训练id    k ISA划分群组数  uid  第几个用户   x 第几个群组
    # 所属群组
    a_group = xlrd.open_workbook(r'/home/user_yao/Movielens/sta_%d/Movielens_isa_%d_%d_all.xlsx'%(f,t,k),'rb')
    gr = a_group.sheet_by_index(x-1)# 评分矩阵

    # 每个群组的用户词典
    group_dict = xlrd.open_workbook(r'/home/user_yao/Movielens/sta_%d/Movielens_isa_%d_%d_sta.xlsx'%(f,t,k),'rb')
...
def ISA_group_inverted(f,t,k):
    ...
    # 创建预测评分矩阵
    workbook = xlsxwriter.Workbook(r'/home/user_yao/Movielens/sta_%d/Movielens_isa_%d_%d_inverted.xlsx'%(f,t,k))
