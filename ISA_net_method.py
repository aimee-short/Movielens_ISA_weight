...
def ISA_group_mid_net(f,t,k):
    ...
    a_group = t_group.sheet_by_name('weight')

    # 每个群组的用户词典
    t_dict = xlrd.open_workbook(r'/home/user_yao/Movielens/sta_%d/Movielens_isa_%d_%d_sta.xlsx'%(f,t,k),'rb')#三行 第一行：用户数项目数 第二行：用户dict 第三行：项目dict
    group_dict = t_dict.sheet_by_name('weight')

    # 1. 创建一个Excel文件
    workbook = xlsxwriter.Workbook(r'/home/user_yao/Movielens/sta_%d/Movielens_isa_%d_%d_weight_net.xlsx'%(f,t,k))
    sheet1 = workbook.add_worksheet('weight_net')
    sheet11 = workbook.add_worksheet('weight_net_all')
