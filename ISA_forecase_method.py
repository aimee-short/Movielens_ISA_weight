...
def ISA_group_mid_Pearson(f,t,k,uid,x):
    ...
    #测试集
    work_test = xlrd.open_workbook(r'/home/user_yao/Movielens/train/Movielens_test_split_65%d.xlsx'%(t),'rb')
    ...
    # 群组均值网络
    t_group = xlrd.open_workbook(r'/home/user_yao/Movielens/sta_%d/Movielens_isa_%d_%d_weight.xlsx'%(f,t,k),'rb')#融合后评分 四行 项目dict 计数 均值 方差
...
def ISA_group_forecast(f,t,k):
    ...
    #是否曾经评过分
    b_rating = xlrd.open_workbook(r'/home/user_yao/Movielens/train/Movielens_array_train_%d.xlsx'%t,'rb')
    table = b_rating.sheet_by_index(0)#通过获取
    nrows = table.nrows
    ncols = table.ncols

    # 每个群组的均值网络
    group_net = xlrd.open_workbook(r'/home/user_yao/Movielens/sta_%d/Movielens_isa_%d_%d_weight_net.xlsx'%(f,t,k),'rb')
    ...
    # 倒排档
    group_inverted = xlrd.open_workbook(r'/home/user_yao/Movielens/sta_%d/Movielens_isa_%d_%d_inverted.xlsx'%(f,t,k),'rb')
    g_inv = group_inverted.sheet_by_name('item_ID')

    # 测试集向量
    work_test = xlrd.open_workbook(r'/home/user_yao/Movielens/train/Movielens_test_split_%d.xlsx'%(t),'rd')
    t_ov = work_test.sheet_by_name('old_vector')#测试所需对比数据 向量
    t_fk = work_test.sheet_by_name('final_top')#最终top-n比对数据
    ...
    # 创建预测评分矩阵
    workbook = xlsxwriter.Workbook(r'/home/user_yao/Movielens/eva_%d/Movielens_isa_%d_%d_weight_forecast.xlsx'%(f,t,k))
